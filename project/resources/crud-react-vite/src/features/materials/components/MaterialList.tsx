...
export default function MaterialList() {
    ...
    const deleteMutation = useMutation({
        mutationFn: (id: number) => deleteObj('/material', id),
        onSuccess: () => {
        ...
        return (
            <Box>
                <Typography color="error">
                    Erreur lors du chargement des matériels : {error.message}
                </Typography>
            </Box>
        );
    }
    ...
    const handleDelete = (id: number) => {
        if (window.confirm('Voulez-vous vraiment supprimer cet matériel ?')) {
            setIsDeleting(true);
    ...
    const filteredMaterials = Array.isArray(objects)
        ? objects.filter(
            (obj) => (obj.designation ?? '').toLowerCase().includes(filter.toLowerCase())
        )
        : [];
    ...
    return (
        <Box>
            ...
            {filteredMaterials.length === 0 ? (
                <Typography>Aucun matériel trouvé.</Typography>
            ) : (
                <Box>
                    <TableContainer component={Paper} elevation={8}>
                        <Table>
                            ...
                            <TableBody>
                                {paginatedMaterials.map((obj) => (
                                    <TableRow key={obj.id}>
                                        <TableCell>{obj.id}</TableCell>
                                        <TableCell>{obj.designation}</TableCell>
                                        <TableCell>
                                            <Button
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2 }}>
                        ...
                        <Typography variant="body2">
                            1 - {paginatedMaterials.length} sur {filteredMaterials.length}
                        </Typography>
                    </Box>
                </Box>
            )}
